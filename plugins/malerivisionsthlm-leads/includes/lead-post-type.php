...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'malerivisionsthlm-leads' ),
					'singular_name' => __( 'Lead', 'malerivisionsthlm-leads' ),
					'edit_item'     => __( 'Förfrågan', 'malerivisionsthlm-leads' ),
					'search_items'  => __( 'Search leads', 'malerivisionsthlm-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'malerivisionsthlm-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'malerivisionsthlm-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'malerivisionsthlm-leads' ),
			'npl_phone'    => __( 'Phone', 'malerivisionsthlm-leads' ),
			'npl_service'  => __( 'Needs', 'malerivisionsthlm-leads' ),
			'npl_timeline' => __( 'When', 'malerivisionsthlm-leads' ),
			'npl_zip'      => __( 'ZIP', 'malerivisionsthlm-leads' ),
			'npl_source'   => __( 'Source', 'malerivisionsthlm-leads' ),
			'npl_status'   => __( 'Status', 'malerivisionsthlm-leads' ),
			'date'         => __( 'Received', 'malerivisionsthlm-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'malerivisionsthlm-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'malerivisionsthlm-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'malerivisionsthlm-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'malerivisionsthlm-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'malerivisionsthlm-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'malerivisionsthlm-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'malerivisionsthlm-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'malerivisionsthlm-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'malerivisionsthlm-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'malerivisionsthlm-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'malerivisionsthlm-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'malerivisionsthlm-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
