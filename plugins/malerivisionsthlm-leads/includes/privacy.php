...
add_filter(
	...
	function ( $exporters ) {
		$exporters['malerivisionsthlm-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'malerivisionsthlm-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'malerivisionsthlm-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'malerivisionsthlm-leads',
						'group_label' => __( 'Quote requests', 'malerivisionsthlm-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['malerivisionsthlm-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'malerivisionsthlm-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
