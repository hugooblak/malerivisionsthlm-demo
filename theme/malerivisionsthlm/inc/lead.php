 ...
 * or photo. They call ma_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional MåleriVision Sthlm AB.
 *
 ...
 * @return string
 */
function ma_sample_note() {
	return ma_lead( 'ar_riktig' ) ? '' : ' <span class="ma-sample">(platshållare)</span>';
}
 ...
 * @return string File name for ma_lead_img().
 */
function ma_lead_bild( $i ) {
	$bilder = (array) ma_lead( 'bilder', array() );
 ...
 * @return string
 */
function ma_ring_label() {
	$namn = ma_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, ma_lead( 'telefon' ) ) : 'Ring ' . ma_lead( 'telefon' );
}
